...
public class PhraseBookTests
{
    ...
    {
        var vocabulary = Vocabulary(spellings: ["TawkType", "GitHub"]);

        Assert.Equal(
            "I pushed TawkType to GitHub",
            PhraseBook.Apply("I pushed tawktype to github", vocabulary).Text);
    }

    [Fact]
    public void A_replacement_turns_what_was_heard_into_what_was_meant()
    {
        var vocabulary = Vocabulary(replacements: [new TextReplacement("tawk type", "TawkType")]);

        Assert.Equal("TawkType is a dictation app", PhraseBook.Apply("tawk type is a dictation app", vocabulary).Text);
    }
    ...
    {
        Assert.Equal(text, PhraseBook.Apply(text, Vocabulary(spellings: ["TawkType"])).Text);
    }
