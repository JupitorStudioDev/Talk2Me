...
public class VocabularyEditTests
{
    ...
    {
        var result = VocabularyEdit.Learn(Existing, "tawk type", "TawkType");

        Assert.True(result.Ok);
        Assert.False(result.Replaced);
        Assert.Equal(2, result.Replacements.Length);
        Assert.Contains(new TextReplacement("tawk type", "TawkType"), result.Replacements);
    }

    [Fact]
    public void Both_halves_are_needed()
    {
        Assert.False(VocabularyEdit.Learn(Existing, "", "TawkType").Ok);
        Assert.False(VocabularyEdit.Learn(Existing, "tawk type", "  ").Ok);
    }
    ...
    public void A_phrase_cannot_be_replaced_with_itself()
        => Assert.False(VocabularyEdit.Learn(Existing, "tawk type", "tawk type").Ok);
