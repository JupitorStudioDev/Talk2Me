...
if (args.Length == 0)
{
    Console.WriteLine("usage: Murmur.Bench <audio.wav> [engine=Both|Parakeet|Whisper] [runs=3] [language=en] [whisperModel=LargeV3Turbo]");
    return 1;
}
...
var settings = new StaticSettings(new MurmurSettings { Model = whisperModel, Language = language });
...
{
    public StaticSettings(MurmurSettings settings) => Current = settings;

    public MurmurSettings Current { get; }
